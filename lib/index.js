...
function getAttributes(attrs, data) {
    ...
    for (let opt in attrs) {
        ...
        if (attrs[opt].indexOf("{") === 0 && attrs[opt].substr(1, 1) !== "{") attrsRet[opt] = myeval('(' + attrs[opt] + ')');
        else if (attrs[opt].indexOf("[") === 0) attrsRet[opt] = myeval('(' + attrs[opt] + ')');
...
function getVar(what, data) {
    ...
    whatSplitted = whatSplitted.map((w) => w.trim());
    let cur;
    // search first in _for
    if (data._for && data._for[whatSplitted[0]] && data._for[whatSplitted[0]] !== undefined) {
        cur = data._for[whatSplitted[0]];
    } else {
        cur = data[whatSplitted[0]];
        if (cur === undefined) {
            return undefined;
        }
    }
...
async function goNode(node, pdf, styles, data) {
    ...
    for (let iChild = 0; iChild < children.length; iChild++) {
        ...
        if (child.name == "declare") {
            // if (attrs.var == "index") {
            //     data._indexFor = attrs.val * 1;
            // } else {
            setVar(attrs.var, data, attrs.val);
        ...
        if (child.name == "for") {
            ...
            if (tab && Array.isArray(tab)) {
                if (!data['_for']) data['_for'] = {};
                if (!data['_for'][child.attr.as]) data['_for'][child.attr.as] = {};

                for (let i = 0; i < tab.length; i++) {
                    data['_for'][child.attr.as] = tab[i];
                    data['_for'][child.attr.as]['_index'] = i;
                    await goNode(child, pdf, styles, data);
                    data['_for'][child.attr.as]['_index']++;
                }
                delete data['_for'][child.attr.as];
            }
        }

        if (child.name == "paths") {
            child.eachChild(function (childPath, indexPath, arrayPath) {
                let attrsPath = getAttributes(childPath.attr, data);
                attrsPath = { ...attrs, ...attrsPath };
                vectorGraphics(pdf, childPath, attrsPath);
